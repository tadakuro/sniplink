...
function SimpleQR({ text, size = 140 }) {
  ...
  for (let r = 0; r < grid; r++) {
    for (let c = 0; c < grid; c++) {
      ...
      if (isFinderZone) {
        cells.push({ r, c, fill: (r < 7 && c < 7) || (r < 7 && c >= grid - 7) || (r >= grid - 7 && c < 7) });
        continue;
      }
      cells.push({ r, c, fill: ((hash * (r * grid + c) * 2654435761) & 1) === 1 });
    }
  }
  const cs = size / grid;
  return (
...
export default function Home() {
  ...
  const baseDisplay = (process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000").replace(/https?:\/\//, "");

  return (
    <>
      <Head>
        <title>Trimly — URL Shortener</title>
        <meta name="description" content="Trim any URL instantly. Free, fast, private." />
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        html, body { background: #080810; color: #e8e8f0; font-family: 'DM Mono', monospace; }
        ...
        .chip { background:#7c3aed22; border:1px solid #7c3aed44; border-radius:6px; color:#a78bfa; font-size:11px; padding:3px 8px; }
        .modal-overlay {
        ...
        .modal-box {
          ...
          padding:28px; text-align:center; max-width:260px; width:90%; animation:popIn .2s ease;
        }
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:640, margin:"0 auto", padding:"48px 20px 80px" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:40 }}>
          <div>
            ...
            <h1 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(1.8rem,5vw,2.8rem)",
            ...
            {user && (
              <p style={{ color:"#44445a", fontSize:12, marginTop:6 }}>
                Hey, {user.firstName || user.emailAddresses?.[0]?.emailAddress} 👋
              </p>
            )}
          </div>
          {/* Clerk UserButton — shows avatar + sign out menu */}
  ...
  />
            ...
            <div style={{ marginTop:24, display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
              {[
                ...
                { n:"03", title:"Share",      desc:"Your links are private — only you can see them" },
              ].map(s => (
                <div key={s.n} style={{ background:"#0a0a14", border:"1px solid #1a1a28", borderRadius:12, padding:"16px 14px" }}>
      ...
      {qrTarget && (
        <div className="modal-overlay" onClick={() => setQrTarget(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ fontSize:13, color:"#a78bfa", marginBottom:4, fontFamily:"'Syne',sans-serif", fontWeight:700 }}>QR Code</div>
            ...
            <div style={{ marginTop:16 }}>
              <button className="btn-ghost" onClick={() => setQrTarget(null)} style={{ width:"100%", justifyContent:"center" }}>Close</button>
            </div>
          </div>
        </div>
      )}
